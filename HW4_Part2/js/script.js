...
$(document).ready(function() {
  ...
  function initializeSlider(inputId, sliderId) {
    ...
    slider.slider({
      ...
      slide: function(event, ui) {
        ...
        // Update table dynamically if form is valid
        if ($("#tableForm").valid()) {
          updateLiveTable();
        }
      }
  ...
  /**
   * Updates the live table in the input tab dynamically
   * This table updates as sliders/inputs change
   */
  function updateLiveTable() {
    ...
    const wrapper = $("#liveTableWrapper");
    wrapper.empty();
  ...
  /**
   * Saves the current table configuration as a new tab
   */
  function saveTableAsNewTab() {
    const hStart = parseInt($("#hStart").val());
    const hEnd = parseInt($("#hEnd").val());
    const vStart = parseInt($("#vStart").val());
    const vEnd = parseInt($("#vEnd").val());
    ...
    const table = generateTableElement(hStart, hEnd, vStart, vEnd);
    const tabContent = `<div id="${tabId}" class="saved-table-tab">${table}</div>`;
  ...
  function updateTabCheckboxes() {
    ...
    // Show/hide delete controls based on whether there are tabs to delete
  ...
  $("#delete-selected").on("click", function() {
    ...
    $("#tabs").tabs("refresh");
    $("#tabs").tabs("option", "active", 0); // Return to input tab
    updateTabCheckboxes();
  });
  ...
  $("#tabs").on("click", "span.ui-icon-close", function() {
    ...
    // Prevent closing the input tab
    if (tabAnchor.index() === 0) {
      alert("Cannot delete the input tab.");
      return;
    }
  ...
  // Initialize with no delete controls visible
  $("#delete-controls").hide();
  
  // Generate initial live preview if default values are valid
  if ($("#tableForm").valid()) {
    updateLiveTable();
  }
});
